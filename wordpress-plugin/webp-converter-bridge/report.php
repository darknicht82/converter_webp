 ...
 */
/**
 * WebP Converter Bridge - Reports Viewer
 * 
 * Muestra los reportes de conversión desde WordPress
...
if (file_exists($wp_log_file)) {
    // Efficiently read only the last 200KB to avoid memory exhaustion on large logs
    $filesize = filesize($wp_log_file);
...
<style>
    /* Scoped styles for WCB Reports */
    .wcb-reports-container {
...
<div class="wcb-reports-container">
    <div class="wcb-header">
        <h1>📊 WebP Converter - Reportes y Estadísticas</h1>
    ...
    <div class="wcb-tabs">
        <button class="wcb-tab-btn active" data-tab="api-logs">Reportes del API</button>
    ...
    <div class="wcb-tab-content active" id="api-logs">
        ...
        <?php if (empty($logs)): ?>
            <div class="wcb-empty">
                <?php if (is_wp_error($response) || wp_remote_retrieve_response_code($response) !== 200): ?>
                    <h3 style="color: #d63638;">Error al obtener reportes</h3>
                    <p>No se pudo conectar con el servidor de reportes.</p>
                <?php else: ?>
                    <h3>No hay reportes disponibles</h3>
                    <p>Aún no se han registrado conversiones para este cliente.</p>
                <?php endif; ?>
            </div>
        <?php else: ?>
    ...
    <div class="wcb-tab-content" id="wp-logs">
        ...
        <?php if (empty($wp_logs)): ?>
            <div class="wcb-empty">
                ...
                <p>No se encontraron entradas recientes relacionadas con el plugin en debug.log.</p>
            </div>
        <?php else: ?>
